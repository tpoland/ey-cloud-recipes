...
when "i686"
  ...
  if node[:environment][:framework] == 'production' && node[:mongo_utility_instances].length < 3
    # this check could be improved as it would not exclude three stand-alone mongos or multiple sets 
    ey_cloud_report "mongodb" do
      message "Unable to setup MongoDB for production; requires a 3 node set."
    end
  else
